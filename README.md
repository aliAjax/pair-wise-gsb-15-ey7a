...
Playwright 会自动启动 Vite（端口 4173），串行验证 Dashboard、编辑器审批配置、双区域校验、条件修复、动态表单预览、发布联动、异常实例详情、异常实例重试闭环、版本比较和历史恢复。测试失败时会保留截图和 trace。
...
8. 在实例详情中重试异常：仅停滞节点的责任角色可发起重试（右上角切换角色验证门禁）；同一实例同一节点同时只有一次重试；重试期间编辑或发布流程不影响实例节点快照；失败保留原异常并追加尝试记录，成功才关闭实例；实例状态与重试记录持久化在 localStorage，刷新后保持一致。
9. 从编辑器进入版本历史，选择两个版本比较，再恢复旧版本为草稿。
